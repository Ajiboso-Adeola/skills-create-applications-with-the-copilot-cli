...
if (require.main === module) {
  ...
  try {
    ...
    if (args.length === 1) {
      ...
      if (['add', 'sub', 'mul', 'div', 'mod', 'pow', 'sqrt', '+', '-', '*', '/', '%', '^', '**'].includes(first)) {
        op = first;
        a = args[1];
        b = args[2];
      } else {
    ...
    let result;
    if (op === '%' || op === 'mod') result = modulo(a, b);
    else if (op === '^' || op === '**' || op === 'pow') result = power(a, b);
    else if (op === 'sqrt') result = squareRoot(a);
    else result = compute(a, op, b);
